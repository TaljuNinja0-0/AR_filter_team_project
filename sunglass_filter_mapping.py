...
def apply_sunglass_filter(frame):
    ...
    for face in faces:
        ...
        glasses_w_3D = eye_distance_3D * 2.1  # 얼굴 폭의 약 2배
        glasses_h_3D = glasses_w_3D * 0.5    # 안경 세로 비율
...
if ext in [".jpg", ".jpeg", ".png"]:
    ...
    result = apply_sunglass_filter(img)
    output_path = os.path.splitext(input_path)[0] + "_sunglass.png"
    cv2.imwrite(output_path, result)
    cv2.imshow("AR Sunglass (Image)", result)
    ...
    out_path = os.path.splitext(input_path)[0] + "_sunglass.mp4"
    ...
    while True:
        ...
        if not ret:
            break
        result = apply_sunglass_filter(frame)
        out.write(result)
        cv2.imshow("AR Sunglass (Video)", result)
